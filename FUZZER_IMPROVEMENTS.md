...
| 6 | **(planned) Optional credential-assisted crypto** — `-K <psk>`; PTK/MIC via bundled `hostapd_lib` (`pbkdf2_sha1` → PMK, `sha1_prf` → PTK, `hmac_sha1` → MIC, key-descriptor v2/AES) | Pass the MIC crypto gate: valid M2 → AP sends M3 → fuzz post-MIC surface (GTK key-data, M3 decrypt path) and M4 | Extends fuzzing reach **past the first crypto-required transition**. Honest split: LLM owns flow + frame *structure*; user supplies the *secret*; a runtime module fills the dynamic crypto fields (live SNonce, MIC) a static corpus can't |
